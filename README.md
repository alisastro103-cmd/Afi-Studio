...
| `api/survey.js` + `survey/template.html` | Satu function gabungan buat 4 kebutuhan survey, dipilih lewat `?mode=`: `page` (render `/survey/` server-side buat og:image, dulu `survey-page.js`), `submit` (terima jawaban, dulu `survey-submit.js`), `results` (agregat hasil khusus admin, dulu `survey-results.js`), `thumbnail` (serve bytes thumbnail buat og:image, dulu `survey-thumbnail.js`). Digabung dari 4 file terpisah supaya gak lewat batas 12 Serverless Function di plan Hobby — URL lama tetap jalan lewat `rewrites` di `vercel.json` |\n| `api/model-page.js` + `model/template.html` | Render halaman `/model/?id=...` (share 1 model) server-side, isi tag `og:*` sesuai data model biar preview link WhatsApp/Discord nampilin thumbnail & judul yang benar. Pola sama persis dengan `api/survey.js` (`?mode=page`) + `survey/template.html`. Fallback ke `Models/models.json` kalau Redis down |
