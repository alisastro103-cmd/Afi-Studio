...
def validate_models(data, reporter):
    ...
    for i, entry in enumerate(data):
        ...
        # thumb sebaiknya jpg/png/webp/gif -- format lain (mis. .avif) sering
        # gagal dirender jadi thumbnail preview waktu link-nya di-share ke
        # WhatsApp/Discord/Telegram (linknya sendiri tetap kebuka normal di
        # browser, cuma preview-nya yang gak muncul). Lihat juga fallback di
        # api/model-page.js yang otomatis skip format ini.
        thumb = entry.get("thumb")
        if thumb and not re.search(r"\.(jpe?g|png|webp|gif)(\?.*)?$", thumb, re.IGNORECASE):
            reporter.warn(
                f"{label} ({name!r}): 'thumb' ({thumb}) bukan format jpg/png/webp/gif -- "
                f"preview link (WhatsApp/Discord) berisiko gak nampilin thumbnail. "
                f"Ganti ke jpg/png/webp lewat Admin Panel."
            )
